...
export default async function SimulatePage({ params }: { params: { id: string } }) {
  ...
  const band = [
    { label: "P10 · downside", value: simulation.p10, tone: "bg-destructive/70" },
    { label: "P50 · expected", value: simulation.p50, tone: "bg-info" },
    { label: "P90 · base case", value: simulation.p90, tone: "bg-ok" },
  ];
  ...
  return (
    <main className="mx-auto max-w-[900px] px-4 py-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <h1 className="text-xl font-semibold">Business case simulation</h1>
        <Badge variant="secondary" className="font-normal">indicative</Badge>
      </div>
      <p className="mt-1 text-sm text-muted-foreground">
        {row.title} — value band over the assumptions in <code>business-case.md</code>.
        The assistant drafts this; a figure here is never committed.
      </p>

      <Card className="mt-5 p-4">
        <h2 className="mb-3 text-sm font-semibold">Value band (annual gross)</h2>
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Downside compounds the untested assumptions; the base case assumes all hold.
          Spread {EUR(simulation.p90 - simulation.p10)} between P10 and P90.
        </p>
      </Card>

      <Card className="mt-4 p-4">
        <h2 className="mb-3 text-sm font-semibold">Assumption sensitivity (tornado)</h2>
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Untested assumptions (amber) are the ones the case rests on. Base figure{" "}
          {facts.annualGross ? EUR(facts.annualGross) : "needs input"} · confidence {facts.confidence ?? "—"}.
        </p>
      </Card>

      <Card className="mt-4 p-4">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-sm font-semibold">Draft section for the pull request</h2>
...
{simulation.draftSection}
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          Appending this opens a pull request against <code>business-case.md</code>. A human reviews and merges — the portal never merges.
        </p>
      </Card>
    </main>
  );
