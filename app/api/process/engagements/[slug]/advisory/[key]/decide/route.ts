...
export async function POST(req: Request, { params }: { params: { slug: string; key: string } }) {
  ...
  if (!proposalId) return NextResponse.json({ error: t("api.process.proposalIdRequired", "proposalId required") }, { status: 400 });
  if (!VERDICTS.includes(verdict)) return NextResponse.json({ error: `${t("api.process.verdictMustBeOneOf", "verdict must be one of")} ${VERDICTS.join(", ")}` }, { status: 400 });
  if (verdict === "rejected" && !String(body.reason || "").trim()) {
    return NextResponse.json({ error: t("api.process.rejectionNeedsReason", "a rejection needs a reason") }, { status: 400 });
  }
