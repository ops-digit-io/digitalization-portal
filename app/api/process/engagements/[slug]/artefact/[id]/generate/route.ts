...
export async function POST(req: Request, { params }: { params: { slug: string; id: string } }) {
  ...
  const prompt = locale === "de" ? "Erzeuge das Artefakt jetzt." : "Generate the artefact now.";
  try {
    const out = await llm.chat(await coach.buildArtefact(slug, id, locale), [{ role: "user", content: prompt }], { maxTokens: 6000 });
