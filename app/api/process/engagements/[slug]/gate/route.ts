...
export async function POST(req: Request, { params }: { params: { slug: string } }) {
  ...
  if (!GATE_KEYS.has(String(body.torId))) return NextResponse.json({ error: t("api.process.noGate", "no such gate") }, { status: 400 });
  if (typeof body.passed !== "boolean") return NextResponse.json({ error: t("api.process.passedBoolean", "passed must be boolean") }, { status: 400 });
  if (!body.passed && !String(body.reason || "").trim()) {
    return NextResponse.json({ error: t("api.process.gateNeedsReason", "a failed gate needs a reason") }, { status: 400 });
  }
