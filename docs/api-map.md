...
Every HTTP endpoint the portal exposes: **63 routes**, grouped by area.
...
graph LR
  ...
  subgraph scout["/api/scout"]
    _api_scout_sweep["/sweep<br/><i>POST</i>"]
  end
...
| `/api/scout/sweep` | POST | `app/api/scout/sweep/route.ts` |
