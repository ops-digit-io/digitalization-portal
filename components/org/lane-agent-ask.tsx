  ...
  return (
    <div className="rounded-lg border p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold">{t("org.askAgent")}</h3>
        <span className="text-xs text-muted-foreground">
          {authority ? `${t("org.scopedTo")} ${authority}` : t("org.noAutonomyWithheld")}
        </span>
      </div>
      ...
      <textarea
        ...
        rows={2}
        placeholder={t("org.askAgentPlaceholder")}
      ...
      {result && (
        <div className="mt-3 border-t pt-3">
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
            <span>{t("org.toolsOffered")} {offered.length ? offered.join(", ") : t("org.none")}</span>
            {withheld.length > 0 && (
              <span>
                {t("org.withheld")} {withheld.map((w) => `${w.name} (${w.reason})`).join("; ")}
              </span>
            )}
          </div>
        </div>
